...
AO08_Sensor::AO08_Sensor(TCA9548A_Mux *mux_ptr, uint8_t muxChannel,
                         uint8_t adsAddress)
...
AO08_Sensor::AO08_Error AO08_Sensor::getLastError() const { return _lastError; }

bool AO08_Sensor::isCalibrated() const {
  return _isCalibratedZero && _isCalibratedAir;
}

void AO08_Sensor::getCalibrationParams(float &voltageZero,
                                       float &voltageAir) const {
...
int16_t AO08_Sensor::readConversionResult() {
  ...
  while (millis() - startTime < 200) { // 200ms 超时
...
bool AO08_Sensor::readVoltage(float &voltage_mV) {
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    if (_lastError == OK) {
      sum_mV += adsValToMillivolts(raw_adc);
      validCount++;
    }
    // readConversionResult 内部包含了等待转换的时间，所以这里不需要额外延时
    // 每次转换大约耗时 8~10ms (128SPS)
  }

  if (validCount == 0) {
    // 如果所有采样都失败，返回 false，_lastError 保留最后一次的错误
    return false;
  }

  voltage_mV = sum_mV / validCount;
  _lastError = OK; // 只要有有效数据就算成功
  return true;
}
